...
export class MapComponent {
  ...
  @Output() zoomed: EventEmitter<any> = new EventEmitter<any>();
  private mapObj;
  ...
  initMap() {
    ...
    google.maps.event.addListener(ctx.mapObj, 'zoom_changed', function (event) {
      ...
      if (zoom < 22 && zoom > 18) { ctx.zoomLevel = 'street'; } // street level
      else if (zoom < 18 && zoom > 12) { ctx.zoomLevel = 'city'; } //city level
      else if (zoom < 12 && zoom > 6) { ctx.zoomLevel = 'country'; } // country levl
      else if (zoom < 5 && zoom > 1) { ctx.zoomLevel = 'world'; } // world level
      ctx.zoomed.emit({ zoomLevel: ctx.zoomLevel, zoomValue: zoom });
      ctx.emitLocation();
